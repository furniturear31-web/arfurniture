...
function NewBillBookForm() {
  ...
  const handleSaveBill = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let prefix = 'AR-INV-'
      if (documentType === 'Quotation') prefix = 'AR-QT-'
      else if (documentType === 'Receipt') prefix = 'AR-RCP-'
      else if (documentType === 'Order Form') prefix = 'AR-ORD-'
      ...
      console.error(error)
      toast.error(error.message || 'Failed to generate bill', { id: toastId })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[#111111]">Create New Bill</h1>
          <p className="text-sm text-gray-500">Fast entry for offline orders and custom furniture.</p>
        </div>
      </div>
          ...
          <div className="bg-gray-900 text-white p-6 rounded-2xl shadow-xl">
            ...
            <div className="mt-8">
              <button disabled={loading} type="submit" className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-gradient-to-r from-[#c8941a] to-[#e9a825] hover:from-[#e9a825] hover:to-[#c8941a] text-black font-bold rounded-xl transition-all shadow-lg hover:shadow-[#c8941a]/30 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed">
                {loading ? 'Saving...' : <><Save size={18} /> Save & Generate Bill</>}
              </button>
            </div>
          </div>
        </div>
